...
class NativeProgramMonitor {
  ...
  async status() {
    ...
    return {
      ...
      reason: !this.gstreamer?.available
        ? 'GStreamer runtime not detected.'
        : !this.gstreamer?.ges
          ? 'GStreamer Editing Services is not installed.'
          : !helper
            ? 'Native Program Monitor helper has not been built yet.'
            : null
    };
  }
  ...
  waitFor(prefix, timeoutMs = 5000) {
    const index = this.pendingLines.findIndex(line => !prefix || line.startsWith(prefix));
    if (index >= 0) {
      const [line] = this.pendingLines.splice(index, 1);
  ...
  async load(project, positionSeconds = 0) {
    ...
    const visualClips = Number(manifest.videoClips || 0) + Number(manifest.titleClips || 0);
    if (!manifest.clips && !manifest.titleClips) {
      return { ok: false, available: true, reason: 'The timeline has no playable media, graphic or caption clips.' };
    }
    ...
    try {
      ...
      if (visualClips > 0) {
        try {
          await Promise.all([this.waitFor('OVERLAY_READY', 3000), this.waitFor('VIDEO_FRAME', 3000)]);
      ...
      if (this.surfaceReady) surface.showInactive(); else surface.hide();
      return {
        ok: true,
        available: true,
        ...
        titleClips: manifest.titleClips,
        graphicClips: manifest.graphicClips,
        visualClips,
  ...
  async seek(seconds) {
    if (!this.ready) return false;
    this.send(`SEEK\t${Math.max(0, Math.round(Number(seconds || 0) * 1e9))}`);
    return true;
  }
  ...
  async position() {
    ...
    try { return Number((await waiting).split('\t')[1] || 0) / 1e9; } catch (_) { return null; }
  }
