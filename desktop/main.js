...
const { renderTimelineProject } = require('./timeline-renderer');
...
function safeAttachment(filePath) {
  const stat = fs.statSync(filePath), ext = path.extname(filePath).toLowerCase();
  const textLike = ['.txt','.md','.json','.csv','.srt','.vtt','.html','.css','.js','.ts','.py','.cpp','.h','.hpp'].includes(ext);
  const imageLike = ['.png','.jpg','.jpeg','.webp','.gif','.bmp','.svg'].includes(ext);
  const videoLike = ['.mp4','.mov','.mkv','.webm','.m4v','.avi'].includes(ext);
  const audioLike = ['.mp3','.wav','.m4a','.aac','.flac','.ogg'].includes(ext);
  let text = null;
  if (textLike && stat.size <= 2 * 1024 * 1024) { try { text = fs.readFileSync(filePath, 'utf8'); } catch (_) {} }
  return { name:path.basename(filePath), path:filePath, url:pathToFileURL(filePath).toString(), size:stat.size, kind:imageLike?'image':videoLike?'video':audioLike?'audio':textLike?'document':'file', text };
}
...
function sanitizeOperations(operations, payload) {
  ...
  return operations.slice(0, 32).flatMap(op => {
    if (!op || !allowed.has(op.type)) return [];
    if (op.type === 'seek' || op.type === 'split_at' || op.type === 'add_marker') return [{ type:op.type, time:clampTime(op.time) }];
    if (op.type === 'remove_range') {
      const start = clampTime(op.start), end = clampTime(op.end);
      if (Math.abs(end - start) < .02) return [];
      return [{ type:op.type, start:Math.min(start,end), end:Math.max(start,end) }];
    }
    if (op.type === 'move_clip') {
      if (!op.clipId) return [];
      return [{ type:op.type, clipId:String(op.clipId), newStart:clampTime(op.newStart) }];
    }
    if (op.type === 'slip_clip' || op.type === 'slide_clip') {
      if (!op.clipId) return [];
      return [{ type:op.type, clipId:String(op.clipId), delta:clampDelta(op.delta) }];
    }
    if (op.type === 'roll_boundary') {
      if (!op.leftId || !op.rightId) return [];
      return [{ type:op.type, leftId:String(op.leftId), rightId:String(op.rightId), delta:clampDelta(op.delta) }];
    }
...
ipcMain.handle('media:pick-many', async () => {
  const r = await dialog.showOpenDialog({ properties:['openFile','multiSelections'], filters:[{ name:'Video', extensions:['mp4','mov','mkv','webm','m4v','avi'] }] });
  if (r.canceled) return [];
  const out = [];
  for (const filePath of r.filePaths.slice(0, 32)) {
    try { out.push(await probeMedia(filePath)); }
    catch (error) { out.push({ path:filePath, name:path.basename(filePath), error:String(error.message || error) }); }
  }
  return out;
});
...
ipcMain.handle('attachment:pick', async () => {
  ...
  if (r.canceled) return [];
  return r.filePaths.slice(0,12).map(safeAttachment);
});
...
ipcMain.handle('project:autosave', async (_e, project) => {
  const target = autosavePath(), temp = `${target}.tmp`;
  fs.writeFileSync(temp, JSON.stringify({ ...project, autosavedAt:new Date().toISOString() }, null, 2), 'utf8');
...
ipcMain.handle('video:export', async (_e, project) => {
  ...
  if (r.canceled || !r.filePath) return null;
  const renderable = project.timeline?.tracks?.some(t => t.kind === 'video' && (t.clips || []).some(c => c.sourcePath && c.duration > 0));
  if (renderable) return renderTimelineProject({ project, outputPath:r.filePath });
  return renderCuts({ sourcePath:project.media?.path, duration:Number(project.media?.duration || 0), removeRanges:project.removeRanges || [], outputPath:r.filePath, frameRate:project.media?.frameRate || project.timeline?.fps || 30 });
});

ipcMain.handle('media:prepare', async (_e, { media, proxy=false }) => {
  if (!media?.path) throw new Error('Load media first.');
  const cacheRoot = path.join(app.getPath('userData'), 'media-cache');
  return prepareMediaArtifacts({ sourcePath:media.path, duration:Number(media.duration || 0), hasAudio:media.hasAudio !== false, cacheRoot, generateProxyFile:Boolean(proxy) });
});
...
ipcMain.handle('transcribe:run', async (_e, { mediaPath, model }) => {
  ...
  const temp = fs.mkdtempSync(path.join(os.tmpdir(),'directorcut-transcript-')), out = path.join(temp,'transcript.json'), py = process.platform === 'win32' ? 'python' : 'python3';
  try {
...
ipcMain.handle('ai:status', async () => {
  const [ollama,gstreamer] = await Promise.all([detectOllama(),detectGStreamer()]);
  return { ollama,gstreamer };
});
...
ipcMain.handle('director:ask', async (_e, payload) => {
  ...
  if (status.running && payload.model) {
    try {
      const result = await chatOllama({ model:payload.model, system, messages:payload.history || [], payload:{ ...payload,workspaceMode,directorPolicy } });
      let operations = sanitizeOperations(result.operations,payload);
      if (workspaceMode !== 'Director' || directorPolicy === 'Ask' || result.intent !== 'edit_task') operations = [];
      return { available:true, ...result, operations, workspaceMode,directorPolicy };
    } catch (error) { return { available:false,provider:'ollama',error:String(error.message||error) }; }
  }
  const endpoint = process.env.DIRECTORCUT_LLM_URL || 'http://127.0.0.1:8080/v1/chat/completions';
  const body = { model:process.env.DIRECTORCUT_LLM_MODEL || 'local-model', temperature:.35, messages:[{role:'system',content:system},...(payload.history||[]).slice(-10),{role:'user',content:JSON.stringify(payload)}] };
  try {
    ...
    return { available:true,provider:'llama.cpp',model:body.model,intent:'conversation',text:json.choices?.[0]?.message?.content || 'No response from local Director model.',operations:[] };
