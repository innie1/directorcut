...
(async()=>{
  ...
  try{
    ...
    const wrappedText=fs.readFileSync(built.textFiles[0],'utf8'),normalizedText=wrappedText.replace(/\s+/g,' ').trim();assert(normalizedText.includes("We're: 100% ready, yes!"));assert(normalizedText.includes('Second line'));
