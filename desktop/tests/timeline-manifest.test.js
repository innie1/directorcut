...
const project = {
  ...
  timeline:{ fps:24, tracks:[
    ...
    { id:'V1', kind:'video', hidden:false, clips:[{ id:'v1', sourcePath:'./sample one.mp4', start:0, sourceIn:1.25, duration:2.5, keyframes:{
      ...
      {id:'color',type:'color',enabled:true,params:{exposure:1.2,contrast:1.1,saturation:1.3,temperature:20,tint:-10,highlights:45,shadows:-30}},
      ...
      {id:'motion-blur',type:'motionBlur',enabled:true,params:{amount:.4}}, {id:'lut',type:'lut',enabled:true,params:{path:'./looks/cinema.cube'}}
    ] }] },
...
const videoFields=videoLine.split('\t'),audioFields=audioLine.split('\t');assert.equal(videoFields.length,28);assert(videoFields[9].includes(`${ns(0)}%3A12`)||decodeURIComponent(videoFields[9]).includes(`${ns(0)}:12`));assert.equal(decodeURIComponent(videoFields[11]),`${ns(0)}:1.1`);assert.equal(decodeURIComponent(videoFields[12]),`${ns(0)}:5`);assert.equal(decodeURIComponent(videoFields[14]),`${ns(0)}:1.25`);assert.equal(decodeURIComponent(audioFields[15]),`${ns(0)}:0.7`);assert.equal(Number(videoFields[16]),1.2);assert.equal(Number(videoFields[17]),1.1);assert.equal(Number(videoFields[18]),1.3);assert.equal(Number(videoFields[19]),20);assert.equal(Number(videoFields[20]),-10);assert.equal(Number(videoFields[21]),4);assert.equal(Number(videoFields[22]),.5);assert.equal(Number(videoFields[23]),.25);assert.equal(Number(videoFields[24]),45);assert.equal(Number(videoFields[25]),-30);assert.equal(Number(videoFields[26]),.4);assert.equal(decodeURIComponent(videoFields[27]),'./looks/cinema.cube');
...
console.log('timeline manifest V6 caption/graphic/Stage 5 effects tests passed');