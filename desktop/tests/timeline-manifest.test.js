...
assert.equal(result.fps, 24);assert.equal(result.clips, 2);assert.equal(result.videoClips, 1);assert.equal(result.audioClips, 1);assert.equal(result.transitions,0);assert.equal(result.nativeTransitionSafe,true);
assert.equal(result.duration, 2.5);assert.equal(result.canvasWidth, 1280);assert.equal(result.canvasHeight, 720);
assert.ok(result.text.startsWith('DIRECTORCUT_TIMELINE_V4\n'));assert.ok(result.text.includes('auto-transition\t0'));assert.ok(result.text.includes('canvas\t1280\t720'));
...
const clipLines = result.text.split('\n').filter(line => line.startsWith('clip\t'));assert.equal(clipLines.length, 2);assert.notEqual(clipLines[0].split('\t')[2], clipLines[1].split('\t')[2]);
const videoFields = clipLines[0].split('\t'),audioFields = clipLines[1].split('\t');assert.equal(videoFields.length, 24);
assert(videoFields[9].includes(`${ns(0)}%3A12`) || decodeURIComponent(videoFields[9]).includes(`${ns(0)}:12`));assert.equal(decodeURIComponent(videoFields[11]), `${ns(0)}:1.1`);assert.equal(decodeURIComponent(videoFields[12]), `${ns(0)}:5`);assert.equal(decodeURIComponent(videoFields[14]), `${ns(0)}:1.25`);assert.equal(decodeURIComponent(audioFields[15]), `${ns(0)}:0.7`);
...
console.log('timeline manifest V4 tests passed');