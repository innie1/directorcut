...
let t=AP.patch(timeline,'v',{gainDb:6,pan:.4,fadeIn:.25,fadeOut:.5,normalize:true,dialogueEnhance:true});
const a=AP.get(t,'v');assert.equal(a.gainDb,6);assert.equal(a.pan,.4);assert.equal(a.fadeIn,.25);assert.equal(a.fadeOut,.5);assert.equal(a.normalize,true);assert.equal(a.dialogueEnhance,true);assert(Math.abs(AP.linearGain(a)-1.995262)<.00001);
...
t=AP.set(t,'v','muted',true);assert.equal(AP.linearGain(AP.get(t,'v')),0);t=AP.reset(t,'v');assert.deepEqual(AP.get(t,'v'),AP.normalize());
