...
function staticProperty(clip, property, defaultValue) {
  const list = Array.isArray(clip.keyframes?.[property]) ? [...clip.keyframes[property]].sort((a,b)=>n(a.time)-n(b.time)) : [];
  return list.length ? n(list[0].value, defaultValue) : defaultValue;
}
function playbackRate(clip) { return clamp(staticProperty(clip,'speed',1),.25,4); }
...
function atempoChain(rate) {
  let value=clamp(rate,.25,4),parts=[];
  while(value>2.000001){parts.push('atempo=2');value/=2;}
  while(value<.499999){parts.push('atempo=0.5');value*=2;}
