...
function normalizedKeyframes(clip, property, defaultValue) {
  ...
  const dedup = new Map();
  for (const k of list) {
    const time = Math.max(0, Math.min(duration, n(k.time)));
    dedup.set(time.toFixed(6), { time, value:n(k.value, defaultValue) });
  }
...
function buildRenderPlan(project = {}) {
  ...
  const videoTracks = (timeline.tracks || [])
    ...
    .sort((a, b) => trackNumber(a.id) - trackNumber(b.id));
  const audioTracks = (timeline.tracks || []).filter(t => t.kind === 'audio' && !t.muted);
  const videoClips = videoTracks.flatMap(t => (t.clips || []).map(c => ({ ...c, _track:t.id }))).filter(c => c.sourcePath && n(c.duration) > 0);
  const audioClips = audioTracks.flatMap(t => (t.clips || []).map(c => ({ ...c, _track:t.id }))).filter(c => c.sourcePath && n(c.duration) > 0);
...
function allocateSourcePads(plan) {
  const videoCounts = new Map(), audioCounts = new Map();
  for (const clip of plan.videoClips) {
    const idx = plan.sourceIndex.get(path.resolve(clip.sourcePath));
    videoCounts.set(idx, (videoCounts.get(idx) || 0) + 1);
  }
  for (const clip of plan.audioClips) {
    const idx = plan.sourceIndex.get(path.resolve(clip.sourcePath));
    audioCounts.set(idx, (audioCounts.get(idx) || 0) + 1);
  }
  const lines = [], videoQueues = new Map(), audioQueues = new Map();
  for (const [idx, count] of videoCounts) {
    const labels = Array.from({ length:count }, (_, i) => `vsrc${idx}_${i}`);
    if (count === 1) lines.push(`[${idx}:v]null[${labels[0]}]`);
    else lines.push(`[${idx}:v]split=${count}${labels.map(x => `[${x}]`).join('')}`);
    videoQueues.set(idx, labels);
  }
  for (const [idx, count] of audioCounts) {
    const labels = Array.from({ length:count }, (_, i) => `asrc${idx}_${i}`);
    if (count === 1) lines.push(`[${idx}:a]anull[${labels[0]}]`);
    else lines.push(`[${idx}:a]asplit=${count}${labels.map(x => `[${x}]`).join('')}`);
    audioQueues.set(idx, labels);
  }
  return {
    lines,
    nextVideo(clip) { const idx = plan.sourceIndex.get(path.resolve(clip.sourcePath)); return videoQueues.get(idx).shift(); },
    nextAudio(clip) { const idx = plan.sourceIndex.get(path.resolve(clip.sourcePath)); return audioQueues.get(idx).shift(); }
  };
}
...
function videoFilterForClip(clip, sourceLabel, label, width, height, fps) {
  ...
  const filters = [
    `[${sourceLabel}]trim=start=${sourceIn.toFixed(6)}:duration=${duration.toFixed(6)}`,
    'setpts=PTS-STARTPTS',
...
function audioFilterForClip(clip, sourceLabel, label) {
  ...
  const filters = [`[${sourceLabel}]atrim=start=${sourceIn.toFixed(6)}:duration=${duration.toFixed(6)}`, 'asetpts=PTS-STARTPTS'];
...
function buildFilterGraph(plan) {
  const { width, height, fps, duration, videoClips, audioClips } = plan;
  const pads = allocateSourcePads(plan);
  const lines = [...pads.lines, `color=c=black:s=${width}x${height}:r=${fps}:d=${duration.toFixed(6)}[base0]`];
  let composite = 'base0';

  videoClips.forEach((clip, i) => {
    const label = `vc${i}`;
    lines.push(videoFilterForClip(clip, pads.nextVideo(clip), label, width, height, fps));
    const next = `vcomp${i}`;
  ...
  audioClips.forEach((clip, i) => lines.push(audioFilterForClip(clip, pads.nextAudio(clip), `ac${i}`)));
...
function fallbackSoftwareArgs(args) {
  const out = args.slice(), encAt = out.indexOf('-c:v');
  if (encAt < 0) return out;
  let end = out.length;
  for (let i = encAt + 2; i < out.length; i++) {
    if (['-pix_fmt','-c:a','-an','-t','-movflags'].includes(out[i])) { end = i; break; }
  }
  out.splice(encAt, end - encAt, '-c:v','libx264','-preset','veryfast','-crf','18');
  return out;
}
...
async function renderTimelineProject({ project, outputPath }) {
  ...
  const temp = fs.mkdtempSync(path.join(os.tmpdir(),'directorcut-render-')), graphFile = path.join(temp,'filter.txt');
  ...
  try {
    ...
    args.push(...encoder.args,'-pix_fmt','yuv420p','-r',String(plan.fps));
    ...
    try {
      ...
      return { outputPath,duration:plan.duration,width:plan.width,height:plan.height,videoClips:plan.videoClips.length,audioClips:plan.audioClips.length,encoder:encoder.name };
    } catch (error) {
      if (encoder.name === 'libx264') throw error;
      await runProcess('ffmpeg',fallbackSoftwareArgs(args));
      return { outputPath,duration:plan.duration,width:plan.width,height:plan.height,videoClips:plan.videoClips.length,audioClips:plan.audioClips.length,encoder:'libx264',hardwareFallback:true };
    }
