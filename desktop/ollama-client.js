...
function normalizeModels(models = []) {return models.map(m=>({name:m.name||m.model,model:m.model||m.name,size:Number(m.size||0),parameterSize:m.details?.parameter_size||null,quantization:m.details?.quantization_level||null,family:m.details?.family||null,modifiedAt:m.modified_at||null})).filter(m=>m.name);}
function cliVersion(){return new Promise(resolve=>{const child=spawn(process.platform==='win32'?'ollama.exe':'ollama',['--version'],{windowsHide:true});let out='';child.stdout?.on('data',d=>out+=d.toString());child.stderr?.on('data',d=>out+=d.toString());child.on('error',()=>resolve(null));child.on('close',code=>resolve(code===0?out.trim():null));});}
async function detectOllama(baseUrl=DEFAULT_BASE){try{const data=await fetchJson(`${baseUrl}/api/tags`,{},900);return{installed:true,running:true,baseUrl,models:normalizeModels(data.models||[])};}catch(error){const version=await cliVersion();return{installed:Boolean(version),running:false,baseUrl,models:[],version,error:String(error.message||error)};}}
async function warmModel(model,baseUrl=DEFAULT_BASE){if(!model)return{ok:false};const data=await fetchJson(`${baseUrl}/api/chat`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({model,messages:[],stream:false,keep_alive:'15m'})},120000);return{ok:true,model:data.model||model,doneReason:data.done_reason||null};}
...
async function visualImages(attachments=[]){
  const images=[],temp=fs.mkdtempSync(path.join(os.tmpdir(),'directorcut-vision-'));
  try{
    for(const a of attachments){if(images.length>=8)break;if(!a?.path)continue;
      if(a.kind==='image'){try{const stat=fs.statSync(a.path);if(stat.isFile()&&stat.size<=20*1024*1024)images.push(fs.readFileSync(a.path).toString('base64'));}catch(_){}continue;}
      if(a.kind==='video'&&images.length<8){try{const media=await probeMedia(a.path),duration=Math.max(.5,Number(media.duration||1)),times=[duration*.25,duration*.7];for(let i=0;i<times.length&&images.length<8;i++){const out=path.join(temp,`frame-${images.length}-${i}.jpg`);await runProcess('ffmpeg',['-y','-ss',times[i].toFixed(3),'-i',a.path,'-frames:v','1','-vf',"scale='min(768,iw)':-2",'-q:v','4',out]);if(fs.existsSync(out))images.push(fs.readFileSync(out).toString('base64'));}}catch(_){}
    }
    return images;
...
async function chatOllama({baseUrl=DEFAULT_BASE,model,system,messages=[],payload,keepAlive='15m'}){
  ...
  const userContent=typeof payload==='string'?payload:JSON.stringify(payload),images=await visualImages(payload?.attachments||[]),schema=outputSchema();
  ...
  let data,visionFallback=false;
  try{data=await postChat(baseUrl,body)}catch(error){if(!images.length)throw error;delete userMessage.images;data=await postChat(baseUrl,body);visionFallback=true;}
