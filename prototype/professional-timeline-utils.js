(function(root,factory){
  ...
  function overwriteMedia(timelineInput,media,time,options={}){
    ...
    const initialVideo=ensureTrack(timeline,'video',options.videoTrackId||'V1','V1 Video'),initialAudio=ensureTrack(timeline,'audio',options.audioTrackId||'A1','A1 Dialogue'),videoTrackId=initialVideo.id,audioTrackId=initialAudio.id,withAudio=media.hasAudio!==false,end=TL.snapTime(at+duration,fps),trackIds=[videoTrackId,...(withAudio?[audioTrackId]:[])];
    ...
    const videoTrack=timeline.tracks.find(t=>t.id===videoTrackId)||ensureTrack(timeline,'video',videoTrackId,'V1 Video'),audioTrack=timeline.tracks.find(t=>t.id===audioTrackId)||ensureTrack(timeline,'audio',audioTrackId,'A1 Dialogue');
