...
(() => {
  ...
  async function warmSelectedModel() {
    if (!window.directorcut?.desktop || typeof window.directorcut.warmModel !== 'function') {
      setAiState('Local AI unavailable','offline');
      return;
    }
    const model = String(state?.selectedModel || '').trim();
    if (!model || model === warmedModel || model === warmingModel) return;
    warmingModel = model;
    setAiState('Starting local AI…','warming');
    composer.classList.add('aiWarming');
    try {
      const result = await window.directorcut.warmModel(model);
      if (warmingModel !== model) return;
      if (result?.ok === false) {
        setAiState('AI starts on send','idle');
      } else {
        warmedModel = model;
        setAiState('AI ready','ready');
      }
    } catch (_) {
      setAiState('AI starts on send','idle');
    } finally {
      if (warmingModel === model) warmingModel = '';
      composer.classList.remove('aiWarming');
      ensureInteractive();
    }
  }
  ...
  function syncWorkingUi() {
    ...
    if (working) {
      send.textContent = '…';
      send.title = 'Director is working';
      if (warmingModel || !warmedModel) setAiState('Working · local AI','working');
      else setAiState('Director working…','working');
    } else {
      send.textContent = '↑';
      send.title = 'Send';
      if (warmedModel) setAiState('AI ready','ready');
      else if (!warmingModel) setAiState('AI readying…','idle');
    }
  }

  // Clicking anywhere in the text area/composer body should immediately focus typing.
  ...
  prompt.addEventListener('focus', () => {
    ensureInteractive();
    if (!warmedModel) warmSelectedModel();
  });
  ...
  // Model discovery is asynchronous at startup. Warm as soon as app.js chooses one.
  warmTimer = setInterval(() => {
    ensureInteractive();
    if (state?.selectedModel && state.selectedModel !== warmedModel && state.selectedModel !== warmingModel) warmSelectedModel();
  }, 700);
